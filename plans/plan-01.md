...
app.scannow.site không implement trong MVP. Chỉ reserve cho tương lai nếu cần central login, tenant switcher, support portal, hoặc user thuộc nhiều nhà hàng.

## Docs Update Plan

Cập nhật bộ docs trong scannow-product-docs để không còn mơ hồ về app.scannow.site.
...
- 02-domain-tenant-deployment-spec.md: đổi app.scannow.site từ “optional” sang “reserved only, not used in MVP”; thêm route map đầy đủ cho owner/manager/staff/kitchen/cashier/customer.
- 03-functional-spec.md: mỗi role phải được mô tả là chạy trong tenant domain, không chạy ở app.scannow.site.
...
- 08-gaps-roadmap.md: cập nhật gap hiện tại: owner/manager có một phần UI, staff/kitchen placeholder, cashier thiếu UI đầy đủ, customer QR ordering thiếu route.
- Tất cả docs phải thống nhất: app.scannow.site = reserved future domain, không phải nơi chứa staff portal trong MVP.

Acceptance docs:

- Search không còn câu nào nói staff/owner/cashier/kitchen nằm ở app.scannow.site trong MVP.
...
- Reserved slugs không được build tenant URL: www, api, admin, app, staging, localhost.
...
- Reserved subdomain app trả null, không resolve tenant.
...
Không thêm route, redirect, hoặc deployment cho app.scannow.site.
...
- app.scannow.site không dùng trong MVP.
